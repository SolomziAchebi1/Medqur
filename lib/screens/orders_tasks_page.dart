  ...
  Widget build(BuildContext context) {
    ...
    return ListView(
      ...
      children: [
        ...
        const Text('Medication instructions are structured, traceable, and immediately visible to the care team.'),
            ...
            Padding(
              ...
              child: SoftCard(
                ...
                child: Column(
                  ...
                  children: [
                    Text(patient.name, style: const TextStyle(fontWeight: FontWeight.w900, color: medqurInk, fontSize: 16)),
                    const SizedBox(height: 8),
                    for (final med in patient.medications)
                      ListTile(
                        contentPadding: EdgeInsets.zero,
                        leading: CircleAvatar(backgroundColor: medqurBlue.withValues(alpha: .10), foregroundColor: medqurBlue, child: const Icon(Icons.medication_outlined)),
                        title: Text('${med.name} • ${med.dose}', style: const TextStyle(fontWeight: FontWeight.w800)),
                        subtitle: Text('${med.route} • ${med.frequency}'),
                        trailing: StatusPill(label: med.administered ? 'Administered' : 'Awaiting nurse', color: med.administered ? medqurGreen : medqurAmber),
                      ),
                  ],
                ),
              ),
  ...
  Widget build(BuildContext context) {
    ...
    for (final patient in patients) {
      for (var i = 0; i < patient.medications.length; i++) {
        if (!patient.medications[i].administered) tasks.add(_Task(patient, patient.medications[i]));
      }
    }
    ...
    return ListView(
      ...
      children: [
        ...
        const SizedBox(height: 6),
        const Text('Confirm the right patient and medication by scanning both before administration.'),
        const SizedBox(height: 22),
        if (tasks.isEmpty)
          const SoftCard(child: Row(children: [Icon(Icons.check_circle_outline, color: medqurGreen), SizedBox(width: 10), Text('No pending medication tasks.')]))
        else
          for (final task in tasks)
            Padding(
              ...
              child: SoftCard(
                ...
                child: Row(
                  children: [
                    CircleAvatar(backgroundColor: medqurAmber.withValues(alpha: .12), foregroundColor: medqurAmber, child: const Icon(Icons.medication_outlined)),
